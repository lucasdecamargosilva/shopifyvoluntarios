(function () {
    ...
    if (window.__PL_VOLUNTARIOS_LOADED__) { console.log('[PL] Widget Ótica Voluntários já carregado — ignorando duplicata.'); return; }
    window.__PL_VOLUNTARIOS_LOADED__ = true;
    console.log('[PL] Widget Ótica Voluntários carregado | URL:', window.location.pathname);
    // ===============================================
    // 0. CHUMBAR A API KEY AQUI DIRETO NO CÓDIGO
    // ===============================================
    const apiKey = "pl_live_dcca13b5e13ff65d54bf9b733fc174f6f5a26ea90bb48ec8";
    ...
    const WEBHOOK_CHECK_LIMIT = 'https://n8n.segredosdodrop.com/webhook/voluntarios-check-limit';
    ...
    function getInstallment() {
        // Shopify Ótica Voluntários: preserva parcelas, valor e juros publicados pelo tema.
...
/* PL: borda arredondada do modal */@media(min-width:768px){.q-card-ia,.q-card,#q-card-ia,#q-card,.q-modal-card{border-radius:16px !important;overflow:hidden;}}
        ...
        .q-btn-trigger-ia:hover { filter: drop-shadow(0 6px 18px rgba(0,0,0,0.32)); animation-play-state: paused; }
    ...
    const html = `
        <div id="q-modal-ia">
            <div class="q-card-ia">
                ...
                <div class="q-content-scroll">
                    ...
                    <div id="q-loading-box">
                        <div class="q-loading-texts">
                            ...
                            <a href="https://provoulevou.com.br/?utm_source=widget&utm_medium=parceiro&utm_campaign=voluntarios" target="_blank" rel="dofollow noopener" class="q-loading-t2">
                ...
                <a href="https://provoulevou.com.br/?utm_source=widget&utm_medium=parceiro&utm_campaign=voluntarios" target="_blank" rel="dofollow noopener" class="q-powered-footer">
                    <span>Powered by</span>
            ...
            try {
                ...
                if (pending) {
                    ...
                    const resp = await fetch(WEBHOOK_PIX, {
                        ...
                        body: JSON.stringify({ email: 'cliente@provoulevou.com.br', phone: '55' + phoneInput.value.replace(/\D/g, ''), loja: 'voluntarios', origin: location.origin })
...
const fd = new FormData();
                ...
                try {
                    ...
                    // Regra Ótica Voluntários: envia todas as fotos do óculos no rosto como referências.
                ...
                console.log('[PL Ótica Voluntários] Enviando', allProdImgs.length, 'fotos do produto');
